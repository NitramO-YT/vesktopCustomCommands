...
4. Inject the content of `vencordDesktopPreload_sample.js` in your Vencord preload file (usually located in `~/.config/Vencord/dist/vencordDesktopPreload.js`). The injection method depends on your Vencord version:
    - **OLD Vencord structure (version 21e6178 or older):** Look for the line `document.addEventListener("DOMContentLoaded",()=>document.documentElement.appendChild(r),{once:!0})` and replace it with `document.addEventListener("DOMContentLoaded",()=>{document.documentElement.appendChild(r);(PRELOAD SAMPLE FILE CONTENT HERE)},{once:!0})`, then replace `(PRELOAD SAMPLE FILE CONTENT HERE)` with the content of `vencordDesktopPreload_sample.js`.
    - **NEW Vencord structure (version c123efd or newer):** Look for the text `getTheme",s.quickCss.getEditorTheme))` and replace it with `getTheme",s.quickCss.getEditorTheme));if(location.protocol!=="data:"){document.readyState==="complete"?(()=>{(PRELOAD SAMPLE FILE CONTENT HERE)})():document.addEventListener("DOMContentLoaded",()=>{(PRELOAD SAMPLE FILE CONTENT HERE)},{once:!0})}`, then replace `(PRELOAD SAMPLE FILE CONTENT HERE)` with the content of `vencordDesktopPreload_sample.js`.
