...
4. Fügen Sie den Inhalt von `vencordDesktopPreload_sample.js` in Ihre Vencord-Preload-Datei ein (normalerweise unter `~/.config/Vencord/dist/vencordDesktopPreload.js`). Die Injektionsmethode hängt von Ihrer Vencord-Version ab:
    - **ALTE Vencord-Struktur (Version 21e6178 oder älter):** Suchen Sie nach der Zeile `document.addEventListener("DOMContentLoaded",()=>document.documentElement.appendChild(r),{once:!0})` und ersetzen Sie sie durch `document.addEventListener("DOMContentLoaded",()=>{document.documentElement.appendChild(r);(PRELOAD SAMPLE FILE CONTENT HIER)},{once:!0})`, ersetzen Sie dann `(PRELOAD SAMPLE FILE CONTENT HIER)` durch den Inhalt von `vencordDesktopPreload_sample.js`.
    - **NEUE Vencord-Struktur (Version c123efd oder neuer):** Suchen Sie nach dem Text `getTheme",s.quickCss.getEditorTheme))` und ersetzen Sie ihn durch `getTheme",s.quickCss.getEditorTheme));if(location.protocol!=="data:"){document.readyState==="complete"?(()=>{(PRELOAD SAMPLE FILE CONTENT HIER)})():document.addEventListener("DOMContentLoaded",()=>{(PRELOAD SAMPLE FILE CONTENT HIER)},{once:!0})}`, ersetzen Sie dann `(PRELOAD SAMPLE FILE CONTENT HIER)` durch den Inhalt von `vencordDesktopPreload_sample.js`.
