...
4. Inyecta el contenido de `vencordDesktopPreload_sample.js` en tu archivo de precarga de Vencord (generalmente ubicado en `~/.config/Vencord/dist/vencordDesktopPreload.js`). El método de inyección depende de tu versión de Vencord:
    - **Estructura ANTIGUA de Vencord (versión 21e6178 o anterior):** Busca la línea `document.addEventListener("DOMContentLoaded",()=>document.documentElement.appendChild(r),{once:!0})` y reemplázala con `document.addEventListener("DOMContentLoaded",()=>{document.documentElement.appendChild(r);(CONTENIDO DEL ARCHIVO DE MUESTRA DE PRECARGA AQUÍ)},{once:!0})`, luego reemplaza `(CONTENIDO DEL ARCHIVO DE MUESTRA DE PRECARGA AQUÍ)` con el contenido de `vencordDesktopPreload_sample.js`.
    - **Estructura NUEVA de Vencord (versión c123efd o posterior):** Busca el texto `getTheme",s.quickCss.getEditorTheme))` y reemplázalo con `getTheme",s.quickCss.getEditorTheme));if(location.protocol!=="data:"){document.readyState==="complete"?(()=>{(CONTENIDO DEL ARCHIVO DE MUESTRA DE PRECARGA AQUÍ)})():document.addEventListener("DOMContentLoaded",()=>{(CONTENIDO DEL ARCHIVO DE MUESTRA DE PRECARGA AQUÍ)},{once:!0})}`, luego reemplaza `(CONTENIDO DEL ARCHIVO DE MUESTRA DE PRECARGA AQUÍ)` con el contenido de `vencordDesktopPreload_sample.js`.
