...
4. Injectez le contenu de `vencordDesktopPreload_sample.js` dans votre fichier de préchargement Vencord (généralement situé dans `~/.config/Vencord/dist/vencordDesktopPreload.js`). La méthode d'injection dépend de votre version de Vencord :
    - **ANCIENNE structure Vencord (version 21e6178 ou antérieure) :** Recherchez la ligne `document.addEventListener("DOMContentLoaded",()=>document.documentElement.appendChild(r),{once:!0})` et remplacez-la par `document.addEventListener("DOMContentLoaded",()=>{document.documentElement.appendChild(r);(CONTENU DU FICHIER D'EXTRAIT DE PRÉCHARGEMENT ICI)},{once:!0})`, puis remplacez `(CONTENU DU FICHIER D'EXTRAIT DE PRÉCHARGEMENT ICI)` par le contenu de `vencordDesktopPreload_sample.js`.
    - **NOUVELLE structure Vencord (version c123efd ou plus récente) :** Recherchez le texte `getTheme",s.quickCss.getEditorTheme))` et remplacez-le par `getTheme",s.quickCss.getEditorTheme));if(location.protocol!=="data:"){document.readyState==="complete"?(()=>{(CONTENU DU FICHIER D'EXTRAIT DE PRÉCHARGEMENT ICI)})():document.addEventListener("DOMContentLoaded",()=>{(CONTENU DU FICHIER D'EXTRAIT DE PRÉCHARGEMENT ICI)},{once:!0})}`, puis remplacez `(CONTENU DU FICHIER D'EXTRAIT DE PRÉCHARGEMENT ICI)` par le contenu de `vencordDesktopPreload_sample.js`.
