...
CONFIG="${XDG_CONFIG_HOME:-$HOME/.config}/vesktopCustomCommands/.env"
[ -f "$CONFIG" ] || exit 0
...
ensure_session_env() {
    local env_file="${XDG_CONFIG_HOME:-$HOME/.config}/vesktopCustomCommands/.env"
    [ -f "$env_file" ] || return 0
...
perform_update_scripts_silent() {
    mkdir -p "${XDG_CONFIG_HOME:-$HOME/.config}/vesktopCustomCommands"
    update_file "$REPO_BASE/dist/vesktopCustomCommands/mute.sh" "${XDG_CONFIG_HOME:-$HOME/.config}/vesktopCustomCommands/mute.sh"; chmod +x "${XDG_CONFIG_HOME:-$HOME/.config}/vesktopCustomCommands/mute.sh" || true
    update_file "$REPO_BASE/dist/vesktopCustomCommands/deafen.sh" "${XDG_CONFIG_HOME:-$HOME/.config}/vesktopCustomCommands/deafen.sh"; chmod +x "${XDG_CONFIG_HOME:-$HOME/.config}/vesktopCustomCommands/deafen.sh" || true
    # Optionally refresh local helpers too
    for helper in vcc-autorepatch.sh vcc-repatch-interactive.sh vcc-autoupdate.sh; do
        update_file "$REPO_BASE/dist/vesktopCustomCommands/$helper" "${XDG_CONFIG_HOME:-$HOME/.config}/vesktopCustomCommands/$helper" 2>/dev/null || true
        chmod +x "${XDG_CONFIG_HOME:-$HOME/.config}/vesktopCustomCommands/$helper" 2>/dev/null || true
    done
}
