    ...
    if prompt_gui_yesno "VCC for Vesktop/Vencord repatch" "A repatch is required. Do you want to apply it now?"; then
        if patch_main "$MAIN_FILE"; then
            if prompt_gui_yesno "VCC for Vesktop/Vencord repatch" "Repatch done. Do you want to restart Vesktop now?"; then
                ...
                restart_vesktop "true" || launch_terminal_cmd "${XDG_CONFIG_HOME:-$HOME/.config}/vesktopCustomCommands/vcc-repatch-interactive.sh"
            fi
        fi
    else
        ...
        if prompt_gui_yesno "VCC for Vesktop/Vencord repatch" "Disable auto-repatch?"; then
            ...
            if [ "$timeTriedGuiPrompt" -lt 3 ]; then
                ...
                # third time :
                launch_terminal_cmd "${XDG_CONFIG_HOME:-$HOME/.config}/vesktopCustomCommands/vcc-repatch-interactive.sh"
            fi
        fi
    fi
}
...
main() {
    ...
    if [ "$AUTO_UPDATE_ACTIVE" = true ] && [ -x "${XDG_CONFIG_HOME:-$HOME/.config}/vesktopCustomCommands/vcc-autoupdate.sh" ]; then
        if [ ! -f "$HOME/.config/systemd/user/vcc-autoupdate.timer" ]; then
            "${XDG_CONFIG_HOME:-$HOME/.config}/vesktopCustomCommands/vcc-autoupdate.sh" || true
        fi
    fi
